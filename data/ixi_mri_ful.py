...
import random
import os
from data.data_preprocessing import pre_procesing, downscale, get_slide_by_index
from torch.utils.data import Dataset
import nibabel as nib

def ixi_get_data_info_and_split(folder_path=config.IXI_FOLDER_PATH_FULL,
                                log_file = None,
                                train_ratio=0.6,
                                val_ratio=0.2,
                                test_ratio=0.2,
...
def ixi_pre_process(path, index, scale=config.DOWN_SCALE_RATE):
    target_tensor = get_slide_by_index(path, index)
...
class IXI_MRI_Dataset_1(Dataset):
    def __init__(self,
                 data_dict,
                 path=config.IXI_FOLDER_PATH_FULL,
                 scan_types=config.IXI_TYPE_FUll,
    ...
    def __getitem__(self, idx):
        ...
        x, y = ixi_pre_process(nii_file, slice_idx, scale=self.scale)
        return x, y
        ...
        for study in self.data_dict:
            for scan_type in self.scan_types:
                ...
                if not os.path.isdir(scan_dir):
                    continue  # bỏ qua nếu study không có scan_type
...
if __name__ == "__main__":
    ...
    train_dict, val_dict, test_dict = ixi_get_data_info_and_split()

    train_ds = IXI_MRI_Dataset_1(train_dict)
    val_ds = IXI_MRI_Dataset_1(val_dict)
    test_ds = IXI_MRI_Dataset_1(test_dict)
